...
import XCTest
    ...
    class SubscriptionTests: XCTestCase {
        ...
        func testAcceptsMultipleSubscriptionFields() async throws {
                ...
                subscription: GraphQLObjectType(
                    ...
                    fields: [
                        "importantEmail": GraphQLField(
                            ...
                            resolve: {
                                emailAny, _, _, eventLoopGroup, _ throws -> EventLoopFuture<Any?> in
                            ...
                            subscribe: {
                                _, _, _, eventLoopGroup, _ throws -> EventLoopFuture<Any?> in
                                return eventLoopGroup.next().makeSucceededFuture(
            ...
            let subscription = try createSubscription(
                schema: schema,
                query: """
                        subscription ($priority: Int = 0) {
                            importantEmail(priority: $priority) {
        ...
        func testSingleSubscriber() async throws {
            let db = EmailDb()
            let subscription = try db.subscription(
                query: """
                        subscription ($priority: Int = 0) {
                            importantEmail(priority: $priority) {
                              email {
        ...
        func testMultipleSubscribers() async throws {
            let db = EmailDb()
            let subscription1 = try db.subscription(
                query: """
                        subscription ($priority: Int = 0) {
                            importantEmail(priority: $priority) {
                              email {
            ...
            let subscription2 = try db.subscription(
                query: """
                        subscription ($priority: Int = 0) {
                            importantEmail(priority: $priority) {
                              email {
