...
extension JSONEncoderImpl: _SpecialTreatmentEncoder {
    ...
    func wrapUntyped(_ encodable: Encodable) throws -> JSONValue {
        ...
        case let object as OrderedDictionary<String, Encodable>:  // this emits a warning, but it works perfectly
...
extension _SpecialTreatmentEncoder {
    @inline(__always) fileprivate func wrapFloat<F: FloatingPoint & CustomStringConvertible>(
        ...
        guard !float.isNaN, !float.isInfinite else {
            if case .convertToString(let posInfString, let negInfString, let nanString) = self
    ...
    fileprivate func wrapEncodable<E: Encodable>(_ encodable: E, for additionalKey: CodingKey?)
        throws -> JSONValue?
    {
        switch encodable {
        case let date as Date:
    ...
    func wrapObject(_ object: OrderedDictionary<String, Encodable>, for additionalKey: CodingKey?)
        throws -> JSONValue
    {
    ...
    _SpecialTreatmentEncoder
{
    ...
    private func _converted(_ key: Key) -> CodingKey {
        ...
        case .useDefaultKeys:
            return key
        case .convertToSnakeCase:
            let newKeyString = GraphQLJSONEncoder.KeyEncodingStrategy._convertToSnakeCase(
                key.stringValue)
...
extension Array where Element == UInt8 {
    ...
    internal static let _false = [
        UInt8(ascii: "f"), UInt8(ascii: "a"), UInt8(ascii: "l"), UInt8(ascii: "s"),
        UInt8(ascii: "e"),
    ]
