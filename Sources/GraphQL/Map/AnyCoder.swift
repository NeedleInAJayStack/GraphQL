...
private struct _AnyKeyedEncodingContainer<K: CodingKey>: KeyedEncodingContainerProtocol {
    ...
    public mutating func superEncoder() -> Encoder {
        return _AnyReferencingEncoder(
            referencing: self.encoder, at: _AnyKey.super, wrapping: self.container)
    }
        ...
        case .custom(let converter):
            self.container = Dictionary(
                container.map {
                    key, value in (
                        converter(
                            decoder.codingPath + [_AnyKey(stringValue: key, intValue: nil)]
                        ).stringValue,
                        value
                    )
                },
    ...
    public mutating func decodeNil() throws -> Bool {
        guard !self.isAtEnd else {
            throw DecodingError.valueNotFound(
                Any?.self,
                DecodingError.Context(
                    codingPath: self.decoder.codingPath + [_AnyKey(index: self.currentIndex)],
...
extension _AnyDecoder {
    ...
    fileprivate func unbox<T>(_ value: Any, as type: _AnyStringDictionaryDecodableMarker.Type)
        throws -> T?
    {
        guard !(value is NSNull) else { return nil }
