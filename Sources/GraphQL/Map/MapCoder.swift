...
extension OrderedDictionary: _MapStringDictionaryEncodableMarker
...
///
private protocol _MapStringDictionaryDecodableMarker {
    static var elementType: Decodable.Type { get }
}

extension OrderedDictionary: _MapStringDictionaryDecodableMarker
where Key == String, Value: Decodable {
...
open class MapEncoder {
    ...
    public struct OutputFormatting: OptionSet {
        ...
        /// Produce human-readable Map with indented output.
    ...
    public enum DateEncodingStrategy {
        ...
        /// Encode the `Date` as a UNIX timestamp (as a Map number).
        case secondsSince1970

        /// Encode the `Date` as UNIX millisecond timestamp (as a Map number).
    ...
    // MARK: - Constructing a Map Encoder
    ...
    open func encode<T: Encodable>(_ value: T) throws -> Map {
        let encoder = _MapEncoder(options: self.options)
...
private class _MapEncoder: Encoder {
    // MARK: Properties

    /// The encoder's storage.
    fileprivate var storage: _MapEncodingStorage

    /// Options set on the top-level encoder.
    fileprivate let options: MapEncoder._Options
    ...
    fileprivate init(options: MapEncoder._Options, codingPath: [CodingKey] = []) {
        self.options = options
        self.storage = _MapEncodingStorage()
    ...
    public func container<Key>(keyedBy: Key.Type) -> KeyedEncodingContainer<Key> {
        ...
        let container = _MapKeyedEncodingContainer<Key>(
            referencing: self,
...
private struct _MapEncodingStorage {
    // MARK: Properties
...
private struct _MapKeyedEncodingContainer<K: CodingKey>: KeyedEncodingContainerProtocol {
    typealias Key = K

    // MARK: Properties

    /// A reference to the encoder we're writing to.
    private let encoder: _MapEncoder
    ...
    fileprivate init(
        referencing encoder: _MapEncoder,
    ...
    private func _converted(_ key: CodingKey) -> CodingKey {
        ...
        case .useDefaultKeys:
            return key
        case .convertToSnakeCase:
            let newKeyString = MapEncoder.KeyEncodingStrategy._convertToSnakeCase(key.stringValue)
            return _MapKey(stringValue: newKeyString, intValue: key.intValue)
    ...
    public mutating func encodeNil(forKey key: Key) throws {
        self.container[_converted(key).stringValue._bridgeToObjectiveC()] = NSNull()
    }
    ...
    public mutating func encode(_ value: Float, forKey key: Key) throws {
        ...
        defer { self.encoder.codingPath.removeLast() }
            ...
            self.container[_converted(key).stringValue._bridgeToObjectiveC()] = try self.encoder
                .box(value)
        #else
    ...
    public mutating func nestedContainer<NestedKey>(
        ...
        let container = _MapKeyedEncodingContainer<NestedKey>(
    ...
    public mutating func superEncoder() -> Encoder {
        return _MapReferencingEncoder(
            referencing: self.encoder, at: _MapKey.super, wrapping: self.container)
    }

    public mutating func superEncoder(forKey key: Key) -> Encoder {
        return _MapReferencingEncoder(referencing: self.encoder, at: key, wrapping: self.container)
    }
}

private struct _MapUnkeyedEncodingContainer: UnkeyedEncodingContainer {
    // MARK: Properties
    ...
    public mutating func encode(_ value: Float) throws {
        // Since the float may be invalid and throw, the coding path needs to contain this key.
        self.encoder.codingPath.append(_MapKey(index: self.count))
        defer { self.encoder.codingPath.removeLast() }
    ...
    public mutating func nestedContainer<NestedKey>(
        ...
        self.codingPath.append(_MapKey(index: self.count))
        defer { self.codingPath.removeLast() }
...
extension _MapEncoder: SingleValueEncodingContainer {
    ...
    fileprivate func box(_ dict: OrderedDictionary<String, Encodable>) throws -> NSObject? {
        ...
        do {
            for (key, value) in dict {
                self.codingPath.append(_MapKey(stringValue: key, intValue: nil))
                defer { self.codingPath.removeLast() }
...
private class _MapReferencingEncoder: _MapEncoder {
    ...
    fileprivate let encoder: _MapEncoder
    ...
    fileprivate init(
        ...
        self.codingPath.append(_MapKey(index: index))
    }
...
open class MapDecoder {
    ...
    public enum DateDecodingStrategy {
        ...
        /// Decode the `Date` as a UNIX timestamp from a Map number.
        case secondsSince1970

        /// Decode the `Date` as UNIX millisecond timestamp from a Map number.
        case millisecondsSince1970
    ...
    // MARK: - Constructing a Map Decoder
    ...
    open func decode<T: Decodable>(_ type: T.Type, from map: Map) throws -> T {
        ...
        let decoder = _MapDecoder(referencing: topLevel, options: self.options)
...
private class _MapDecoder: Decoder {
    // MARK: Properties

    /// The decoder's storage.
    fileprivate var storage: _MapDecodingStorage

    /// Options set on the top-level decoder.
    fileprivate let options: MapDecoder._Options
    ...
    fileprivate init(
        ...
        options: MapDecoder._Options
    ) {
        self.storage = _MapDecodingStorage()
    ...
    public func container<Key>(keyedBy type: Key.Type) throws -> KeyedDecodingContainer<Key> {
        ...
        let container = _MapKeyedDecodingContainer<Key>(referencing: self, wrapping: topContainer)
    ...
    public func unkeyedContainer() throws -> UnkeyedDecodingContainer {
        ...
        return _MapUnkeyedDecodingContainer(referencing: self, wrapping: topContainer)
    }
...
private struct _MapDecodingStorage {
    // MARK: Properties

    /// The container stack.
    /// Elements may be any one of the Map types (NSNull, NSNumber, String, Array, [String : Any]).
...
private struct _MapKeyedDecodingContainer<K: CodingKey>: KeyedDecodingContainerProtocol {
    typealias Key = K

    // MARK: Properties

    /// A reference to the decoder we're reading from.
    private let decoder: _MapDecoder
    ...
    fileprivate init(referencing decoder: _MapDecoder, wrapping container: [String: Any]) {
        ...
        case .custom(let converter):
            self.container = Dictionary(
                container.map {
                    key, value in
                    (
                        converter(decoder.codingPath + [_MapKey(stringValue: key, intValue: nil)])
                            .stringValue, value
                    )
                },
                uniquingKeysWith: { (first, _) in first }
    ...
    private func _errorDescription(of key: CodingKey) -> String {
        ...
        case .convertFromSnakeCase:
            ...
            let converted = MapEncoder.KeyEncodingStrategy._convertToSnakeCase(original)
    ...
    public func nestedContainer<NestedKey>(
        ...
        let container = _MapKeyedDecodingContainer<NestedKey>(
    ...
    public func nestedUnkeyedContainer(forKey key: Key) throws -> UnkeyedDecodingContainer {
        ...
        guard let value = self.container[key.stringValue] else {
            throw DecodingError.keyNotFound(
                ...
                DecodingError.Context(
                    codingPath: self.codingPath,
                    debugDescription:
                        "Cannot get UnkeyedDecodingContainer -- no value found for key \(_errorDescription(of: key))"
        ...
        return _MapUnkeyedDecodingContainer(referencing: self.decoder, wrapping: array)
    }
    ...
    public func superDecoder() throws -> Decoder {
        return try _superDecoder(forKey: _MapKey.super)
    }
...
private struct _MapUnkeyedDecodingContainer: UnkeyedDecodingContainer {
    // MARK: Properties
    ...
    /// Initializes `self` by referencing the given decoder and container.
    fileprivate init(referencing decoder: _MapDecoder, wrapping container: [Any]) {
        self.decoder = decoder
        self.container = container
        self.codingPath = decoder.codingPath
        self.currentIndex = 0
    ...
    public mutating func decodeNil() throws -> Bool {
        guard !self.isAtEnd else {
            throw DecodingError.valueNotFound(
                Any?.self,
                DecodingError.Context(
                    codingPath: self.decoder.codingPath + [_MapKey(index: self.currentIndex)],
    ...
    public mutating func decode(_ type: Bool.Type) throws -> Bool {
        ...
        self.decoder.codingPath.append(_MapKey(index: self.currentIndex))
        defer { self.decoder.codingPath.removeLast() }

        guard let decoded = try self.decoder.unbox(self.container[self.currentIndex], as: Bool.self)
        else {
            throw DecodingError.valueNotFound(
                type,
                DecodingError.Context(
    ...
    public mutating func decode(_ type: Int.Type) throws -> Int {
        ...
        guard let decoded = try self.decoder.unbox(self.container[self.currentIndex], as: Int.self)
        else {
            throw DecodingError.valueNotFound(
                type,
                DecodingError.Context(
    ...
    public mutating func decode(_ type: Int8.Type) throws -> Int8 {
        ...
        guard let decoded = try self.decoder.unbox(self.container[self.currentIndex], as: Int8.self)
        else {
            throw DecodingError.valueNotFound(
                type,
                DecodingError.Context(
    ...
    public mutating func decode(_ type: UInt.Type) throws -> UInt {
        ...
        guard let decoded = try self.decoder.unbox(self.container[self.currentIndex], as: UInt.self)
        else {
            throw DecodingError.valueNotFound(
                type,
                DecodingError.Context(
    ...
    public mutating func decode<T: Decodable>(_ type: T.Type) throws -> T {
        ...
        guard let decoded = try self.decoder.unbox(self.container[self.currentIndex], as: type)
        else {
            throw DecodingError.valueNotFound(
                type,
                DecodingError.Context(
...
extension _MapDecoder: SingleValueDecodingContainer {
...
extension _MapDecoder {
    ...
    fileprivate func unbox(_ value: Any, as type: Int.Type) throws -> Int? {
        ...
        guard let number = __SwiftValue.store(value) as? NSNumber, number !== kCFBooleanTrue,
        ...
        guard NSNumber(value: int) == number else {
            throw DecodingError.dataCorrupted(
                DecodingError.Context(
                    codingPath: self.codingPath,
                    debugDescription: "Parsed Map number <\(number)> does not fit in \(type)."
                )
            )
        }
    ...
    fileprivate func unbox(_ value: Any, as type: Float.Type) throws -> Float? {
        ...
        if let number = __SwiftValue.store(value) as? NSNumber, number !== kCFBooleanTrue,
            number !== kCFBooleanFalse
        {
            ...
            case .convertFromString(let posInfString, let negInfString, let nanString) = self
    ...
    fileprivate func unbox<T>(_ value: Any, as type: _MapStringDictionaryDecodableMarker.Type)
        throws -> T?
    {
        guard !(value is NSNull) else { return nil }
...
private struct _MapKey: CodingKey {
    ...
    fileprivate static let `super` = _MapKey(stringValue: "super")!
}
...
extension EncodingError {
    ...
    fileprivate static func _invalidFloatingPointValue<T: FloatingPoint>(
        ...
        let debugDescription =
            "Unable to encode \(valueDescription) directly in Map. Use MapEncoder.NonConformingFloatEncodingStrategy.convertToString to specify how the value should be encoded."
        return .invalidValue(
