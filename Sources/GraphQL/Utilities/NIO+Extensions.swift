...
extension Collection {
    public func flatten<T>(on eventLoopGroup: EventLoopGroup) -> Future<[T]>
    where Element == Future<T> {
...
extension OrderedDictionary where Value: FutureType {
    func flatten(
        ...
        return EventLoopFuture
            .whenAllSucceed(futures, on: eventLoopGroup.next())
...
public protocol FutureType {
    ...
    func map<NewValue>(
        file: StaticString, line: UInt, _ callback: @escaping (Expectation) -> (NewValue)
